...
def self_train_SVM(X1, Y1, X2, label, labeled_index, unlabeled_index, news_user_matrix, X2_label, C=1, kernel='linear', max_epoch=30):
    ...
    label_v = (0*(np.multiply(news_user_matrix, Wc).T) +  1*(label_c)).T
    ...
    a[np.where(a>2)]=1
    print(a)
    np.savetxt("user_clust——liar.txt", a, fmt="%d", delimiter=" ")
    #a=np.loadtxt("user_clust.txt")
    ...
    #np.savetxt("model_new__0.txt",test_accuracy,fmt="%f", delimiter=",")
    #np.savetxt("model_new_f1_0.txt", test_f1, fmt="%f", delimiter=",")
    #np.savetxt("model_new_precision_0.txt",  test_precision, fmt="%f", delimiter=",")
    #np.savetxt("model_new_recall_0.txt",test_recall, fmt="%f", delimiter=",")
    #np.savetxt("model_new_aucroc_0.txt", test_aucroc, fmt="%f", delimiter=",")
...
l=self_train_SVM(X1, Y1, X2, label, labeled_index, unlabeled_index, news_user_matrix, X2_label, C=1, kernel='linear', max_epoch=1)
'''for i in range(7):
    n=n+0.1
