...
def self_train_SVM(X1, Y1, X2, label, labeled_index, unlabeled_index, news_user_matrix, X2_label, C=1, kernel='linear', max_epoch=200, t_method=None):
    ...
    thre = get_thre(Y3_real, Y4, 0.75,0)
...
def self_train_SVM_without_w(X1, Y1, X2, X2_label, C=1, kernel='linear', max_epoch=20):
    l = X1.shape[0]
    u = X2.shape[0]
    N = l + u
    acc_iter_list = []
...
a=open("result_with_user_阈值为0.75不加卡尔曼滤波不加数据增强.txt","w")
