...
def model(train,test,train_label,test_label):
    ...
    model_fit = model.fit(train, train_label, batch_size=128, epochs=10,
...
h=np.loadtxt('sentence2Vec_3.txt')
...
unlabeled_index = index[: int(len(h) * 0.2)]
labeled_index = index[int(len(h) * 0.2):]
...
model=model(train,test,train_label,test_label)
train = np.array(train)
#test = test.readlines()
#test = [i.strip().split(",") for i in test]
test = np.array(test)
