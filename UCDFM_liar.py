...
def self_train_SVM(X1, Y1, X2, label, labeled_index, unlabeled_index, news_user_matrix, X2_label, C=1, kernel='linear', max_epoch=30):
    ...
    label_v = (0.5*(np.multiply(news_user_matrix, Wc).T) +0.5*(label_c)).T
    ...
    a[np.where(a>2)]=1
    print(a)
    np.savetxt("user_clust2.txt", a, fmt="%d")'''
    a=np.loadtxt("user_liar_matrix.txt")
    ...
    np.savetxt("model_liar_acc_boltzmann_without_clust.txt",test_accuracy,fmt="%f", delimiter=",")
    np.savetxt("model_liar_f1_liar_boltzmann_without_clust.txt", test_f1, fmt="%f", delimiter=",")
    np.savetxt("model_liar_precision_liar_boltzmann_without_clust.txt",  test_precision, fmt="%f", delimiter=",")
    np.savetxt("model_liar_recall_liar_boltzmann_without_clust.txt",test_recall, fmt="%f", delimiter=",")
    np.savetxt("model_liar_aucroc_liar_boltzmann_without_clust.txt", test_aucroc, fmt="%f", delimiter=",")
