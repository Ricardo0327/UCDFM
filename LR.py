...
def text_cnn(x_train,y_train,x_test,y_test):
    ...
    for fsz in filter_sizes:
        l_conv = Conv1D(filters=100, kernel_size=fsz, activation='relu')(tensor_input)#kernel_size中只用说明窗口大小就行,不用说宽度是多少,因为宽度默认是embedding size
    ...
    print('precision of testing set :', score[2])
    print('recall of testing set :', score[3])
    print('f1 score of testing set :', score[4])
    return model

def lstm(x_train,y_train,x_test,y_test):
    main_input=Input(shape=(60,10))
    lstm=LSTM(units=25,return_sequences=True,name='dense1')(main_input)#默认了activation function是tanh,recurrent_activation function是hard_sigmoid
    x=Lambda(l_average)(lstm)
    print(K.int_shape(x))
    ...
    print(K.int_shape(predictions))
    model=Model(main_input,predictions)#输入是(30,60,10)输出是(30,2)
    model.compile(loss="binary_crossentropy", optimizer="adam", metrics=['binary_accuracy'])
    model.fit(x_train,y_train,batch_size=30, epochs=20)
    score = model.evaluate(x_test, y_test)
    print('total loss on testing set :', score[0])
...
def self_train_SVM(X1, Y1, X2, Y2, label, labeled_index, unlabeled_index, news_user_matrix, X2_label, C=1, kernel='linear', max_epoch=200, t_method=None):
    l = X1.shape[0]
    u = X2.shape[0]
    N = l + u
    ...
    #print(Y2.shape)
    #print('*******************************************************')
    eval = clf.evaluate(X2,X2_label)
    eval1 = clf.evaluate(X1,Y1)
    l_results = clf.predict(X1)
    u_results = clf.predict(X2)
    #clf.fit(X1, Y1)
    print(l_results)
    print("Evaluation on test data: loss = %0.6f accuracy = %0.2f%% \n" \
          % (eval1[0], eval1[1] ))
    #print("Evaluation on test1 data: loss = %0.6f accuracy = %0.2f%% \n" \
       #   % (eval ))
    print(eval)
    #np.savetxt("acuracy.txt",eval,fmt="%f", delimiter=",")'''
...
history = model.fit(X1,
                    ...
                    validation_data=(X2,X2_label))
results = model.evaluate(X2, X2_label)
print(results)

'''a=open("result_with_user_lr.txt","w")
